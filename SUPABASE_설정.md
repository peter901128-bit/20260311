...
**배포 없이 로컬에서만 테스트할 때:** 화면의 **로컬 설정 (배포 없이 저장 테스트용)**을 펼친 뒤, Supabase URL과 Anon Key를 입력하고 **저장**하면 이 기기 브라우저에만 저장되어, 그 다음부터 번호 뽑기 시 Supabase에 저장됩니다.
...
- **"연결 설정 없음"이 떠요**  
  - **로컬에서 열었을 때:** 화면에서 **로컬 설정 (배포 없이 저장 테스트용)**을 펼치고, Supabase **Project URL**과 **anon public** 키를 입력한 뒤 **저장**하세요. (Supabase 대시보드 → Project Settings → API에서 확인)  
  - 또는 **Vercel에 배포된 주소**로 접속하면 서버 설정이 적용되어 별도 입력 없이 저장됩니다.
- **몇 개만 쌓이다가 갑자기 안 쌓여요**  
